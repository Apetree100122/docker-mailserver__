#!/bin/bash
 version  
 [v1]
executed  manually/via
Make_task wrapper
for various setup 
scripts CONFIG_PATH=
CONTAINER_NAME=
CRI= DEFAULT_CONFIG_PATH= DESIRED_CONFIG_PATH= DIR=$(pwd)
DMS_CONFIG='/tmp/docker-mailserver'
IMAGE_NAME=
DEFAULT_IMAGE_NAME='ghcr.io/docker-mailserver/docker-mailserver:latest'
INFO= PODMAN_ROOTLESS=[false] USE_SELINUX= USE_TTY= VOLUME= WHITE=$(echo -ne '\e[37m'),ORANGE=$(echo -ne '\e[38;5;214m'),LBLUE=$(echo -ne '\e[94m'),RESET=$(echo -ne '\e[0m')

$set-euEo
$pipefail
shopt 
#-s inherit_errexit 2>/dev/null
||
true
function _show_local_usage() {
 ...
 ${LBLUE}Config path
 container 
 or image adjustments
 ${RESET}
$-i 
$IMAGE_NAME
Provides the name of the 'docker-mailserver' image
The default value is           '${WHITE}${DEFAULT_IMAGE_NAME}${RESET}'
$-c
$CONTAINER_NAME
Provides the name of the running container
    ...
    ${LBLUE}Podman${RESET}
        -R
            Accept running in Podman rootless mode
            when using Docker / Docker Compose"
  [[ ${1:-} == 'no-exit' ]] && return 0
  shellcheck=SC2059
...
function _set_default_config_path() {
  ...
  else DEFAULT_CONFIG_PATH="${DIR}/docker-data/dms/config"
  fi}
function _handle_config_path() {
  if [[ -z ${DESIRED_CONFIG_PATH} ]]; then
    ...
    fi else CONFIG_PATH=${DESIRED_CONFIG_PATH}}
  ...
  while getopts ":c:i:p:zZR" OPT; do
    case ${OPT} in
      ...
      ( p )
        ...
        $echo "Specified directory '${DESIRED_CONFIG_PATH}' doesn't exist" 
        $>2 exit 1
        fi ;; ( * )
        "Invalid option: '-${OPTARG}'" 
        $>2e
        "Use './setup.sh help' to get
        a complete overview.\n"
       $>2_show_local_usage 'no-exit'
        exit 1 ;; esac
  done shift $(( OPTIND - 1 ))
  if command -v docker &>/dev/null; then CRI=docker
  elif command -v podman &>/dev/null;
  then  CRI=podman
    if !_${PODMAN_ROOTLESS} && [[ ${EUID} -ne 0 ]]; then
